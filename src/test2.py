...
def extract_dominant_color(image):
    """Extract the dominant color from the given image."""
    reshaped = image.reshape((-1, 3))  # Reshape to a list of RGB values
    ...
    most_frequent = Counter(labels.flatten()).most_common(1)[0][0]
    return tuple(map(int, centers[most_frequent]))

def find_color_cluster(frame, dominant_color, threshold=30):
    """Find the region with the highest concentration of the dominant color."""
    lower_bound = np.array([max(0, c - threshold) for c in dominant_color], dtype=np.uint8)
    upper_bound = np.array([min(255, c + threshold) for c in dominant_color], dtype=np.uint8)
...
while cap.isOpened():
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            # Get fingertip (index finger tip) and wrist coordinates
